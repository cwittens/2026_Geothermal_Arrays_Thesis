...
# Only now multiple boreholes and deeper (3000m)

grid_points = Dict(
    1 => [0.0],
    2 => [-30.0, 30.0],
    3 => [-60.0, 0.0, 60.0],
    4 => [-60.0, -30.0, 30.0, 60.0],
    5 => [-60.0, -30.0, 0.0, 30.0, 60.0],
    6 => [-90.0, -60.0, -30.0, 30.0, 60.0, 90.0],
    7 => [-90.0, -60.0, -30.0, 0.0, 30.0, 60.0, 90.0],
    8 => [-120.0, -90.0, -60.0, -30.0, 30.0, 60.0, 90.0, 120.0],
    9 => [-120.0, -90.0, -60.0, -30.0, 0.0, 30.0, 60.0, 90.0, 120.0],
    10 => [-150.0, -120.0, -90.0, -60.0, -30.0, 30.0, 60.0, 90.0, 120.0, 150.0],
)
...
for (nx, ny) in configs
    ...
    tspan_full = (0, 3600 * 24 * 10)  # 10 days [s]
    saveat_full = range(tspan_full..., 2)

    T0, tspan, saveat = T0_fresh, tspan_full, saveat_full
    ...
    # pre compile
    println("Pre-compiling run...")
    prob_short = ODEProblem(rhs_diffusion_z!, T0, (0, 320), cache)
    CUDA.@profile solve(
        prob_short,
    ...
    push!(profiling_results, string(prof))

    println("Completed configuration with $(length(XC))×$(length(YC)) borehole array")
...
@save joinpath(simulation_dir(), "benchmarking_kernel_results.jld2") profiling_results
